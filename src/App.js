...
const fmt = (n) =>
  new Intl.NumberFormat("kk-KZ", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(n || 0) + " ₸";
...
export default function App() {
  ...
  // ── ADMIN ─────────────────────────────────────────────────────────────────
  const handleAdminGive = async (subtract = false) => {
    const amt = parseFloat(adminGive.amount);
    if (!adminGive.username || !amt) return notify("Заполните поля", "error");
    try {
      const [target] = await sb(`users?username=eq.${adminGive.username}&select=*`);
      ...
      await sb(`users?username=eq.${adminGive.username}`, { method: "PATCH", body: JSON.stringify({ balance: newBal }) });
      await sb("transactions", { method: "POST", prefer: "return=minimal", body: JSON.stringify({ id: genId(), username: adminGive.username, type: subtract ? "out" : "bonus", amount: subtract ? -amt : amt, description: `👑 ${subtract ? "Списано" : "Начислено"} администратором`, date: nowStr() }) });
      notify(`${subtract ? "Списано" : "Начислено"} ${fmt(amt)}`);
      setAdminGive({ username: "", amount: "" }); loadAllUsers();
    } catch (e) { notify(e.message, "error"); }
  };

  const viewUserTx = async (username) => {
    const txs = await sb(`transactions?username=eq.${username}&order=created_at.desc&limit=20`);
    setAdminUserTx(txs); setSelectedUser(username);
  };
        ...
        <div style={S.overlay} onClick={() => setDrawerOpen(false)}>
          <div style={S.drawer} onClick={e => e.stopPropagation()}>
            <div style={S.drawerHeader}>
              <div style={{ ...S.avatar, background: isAdmin ? "linear-gradient(135deg,#f9ca24,#f0932b)" : "linear-gradient(135deg,#00b894,#00d68f)" }}>
                {user.fullname[0]}
              </div>
              <div>
            ...
            {[
              ...
              <button key={p} style={S.drawerBtn} onClick={() => navigate(p)}>
        ...
        {page === "home" && (
          <div>
            ...
            <div style={S.earnCard}>
              <div style={S.earnTop}>
                <div>
                  <div style={S.secTitle}>💰 Клик-заработок</div>
                  <div style={S.earnDesc}>1 клик = 1 ₸ • лимит 100 кликов</div>
                </div>
        ...
        {page === "transfer" && (
          <div>
            ...
            <div style={S.card}>
              ...
              <input style={S.inp2} placeholder="username" value={tfForm.to} onChange={e => setTfForm({ ...tfForm, to: e.target.value })} />
              <Label>Сумма (₸)</Label>
          ...
          <div>
            ...
            <div style={S.card}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button style={{ ...S.btnPrimary, flex: 1 }} onClick={() => handleAdminGive(false)}>➕ Начислить</button>
                <button style={{ ...S.btnPrimary, flex: 1, background: "linear-gradient(135deg,#ff4d4d,#ff6b6b)" }} onClick={() => handleAdminGive(true)}>➖ Списать</button>
              </div>
            </div>
            <div style={S.secTitle}>Все пользователи ({allUsers.length})</div>
            <div style={S.txCard}>
              {allUsers.map(u => (
                <div key={u.username} style={{ ...S.txRow, flexWrap: "wrap" }}>
                  <div style={{ ...S.avatar, width: 38, height: 38, fontSize: 15, flexShrink: 0, background: u.is_admin ? "linear-gradient(135deg,#f9ca24,#f0932b)" : "linear-gradient(135deg,#00b894,#00d68f)" }}>{u.fullname[0]}</div>
                  <div style={{ flex: 1 }}>
                    <div style={S.txDesc}>{u.fullname} {u.is_admin && <span style={S.adminBadge}>ADMIN</span>}</div>
                    <div style={S.txDate}>@{u.username}</div>
                  </div>
                  <div style={{ color: "#00d68f", fontWeight: 700, fontSize: 13 }}>{fmt(u.balance)}</div>
                  <button style={{ ...S.btnRed, width: "auto", padding: "6px 12px", fontSize: 12, marginTop: 0, marginLeft: 8 }} onClick={() => viewUserTx(u.username)}>📋</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={S.modalOverlay}>
          <div style={S.modal}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <b style={{ color: "#e8eaf0" }}>@{selectedUser}</b>
...
const S = {
  ...
  logoText: { fontSize: 20, letterSpacing: 2 },
...
"home");
  ...
  useEffect(() => { if (page === "credit" && user) loadCredits(); }, [page, user]);
  ...
  const handleCredit = async () => {
    ...
    try {
      ...
      await sb("credits", {
        ...
        body: JSON.stringify({ id, username: user.username, amount: amt, remaining: parseFloat((monthly * months).toFixed(2)), monthly, months, paid: 0, active: true, date: nowStr() })
      });
      await sb(`users?username=eq.${user.username}`, {
        method: "PATCH",
        body: JSON.stringify({ balance: user.balance + amt })
      });
      await sb("transactions", {
        method: "POST", prefer: "return=minimal",
        body: JSON.stringify({ id: genId(), username: user.username, type: "credit", amount: amt, description: `💳 Кредит одобрен на ${months} мес.`, date: nowStr() })
      });
      setUser(u => ({ ...u, balance: u.balance + amt }));
      setCrForm({ amount: "", months: "12" });
      setCrError("");
      setLoading(false);
      notify(`Кредит одобрен! ${fmt(amt)} зачислено`);
      loadCredits();
  ...
  const handlePayCredit = async (credit) => {
    ...
    try {
      const newRemaining = parseFloat((credit.remaining - pay).toFixed(2));
      await sb(`credits?id=eq.${credit.id}`, {
        method: "PATCH",
        body: JSON.stringify({ remaining: newRemaining, paid: credit.paid + 1, active: newRemaining > 0 })
      });
      await sb(`users?username=eq.${user.username}`, {
        method: "PATCH",
        body: JSON.stringify({ balance: user.balance - pay })
      });
      await sb("transactions", {
        method: "POST", prefer: "return=minimal",
        body: JSON.stringify({ id: genId(), username: user.username, type: "out", amount: -pay, description: `💳 Оплата кредита #${credit.id}`, date: nowStr() })
      });
      setUser(u => ({ ...u, balance: u.balance - pay }));
      setLoading(false);
      notify(`Оплачено ${fmt(pay)}`);
      loadCredits();
    } catch (e) { setLoading(false); notify(e.message, "error"); }
  };
  ...
  useEffect(() => { if (page === "deposit" && user) loadDeposits(); }, [page, user]);
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      await sb("deposits", {
        method: "POST", prefer: "return=minimal",
        body: JSON.stringify({ id, username: user.username, amount: amt, months, rate: rate * 100, profit, active: true, date: nowStr() })
      });
      await sb(`users?username=eq.${user.username}`, {
        method: "PATCH",
        body: JSON.stringify({ balance: user.balance - amt })
      ...
      await sb("transactions", {
        method: "POST", prefer: "return=minimal",
        body: JSON.stringify({ id: genId(), username: user.username, type: "out", amount: -amt, description: `🏦 Открыт вклад на ${months} мес.`, date: nowStr() })
      });
      setUser(u => ({ ...u, balance: u.balance - amt }));
      setDepForm({ amount: "", months: "6" });
      setDepError("");
      setLoading(false);
      notify(`Вклад открыт! Доход: +${fmt(profit)}`);
      loadDeposits();
    } catch (e) { setLoading(false); setDepError(e.message); }
  };
  ...
  const handleCloseDeposit = async (dep) => {
    ...
    setLoading(true);
    try {
      await sb(`deposits?id=eq.${dep.id}`, {
        method: "PATCH",
      ...
      await sb("transactions", {
        method: "POST", prefer: "return=minimal",
        body: JSON.stringify({ id: genId(), username: user.username, type: "in", amount: total, description: `🏦 Закрытие вклада #${dep.id}`, date: nowStr() })
      });
      setUser(u => ({ ...u, balance: u.balance + total }));
      setLoading(false);
      notify(`Получено ${fmt(total)}`);
      loadDeposits();
    } catch (e) { setLoading(false); notify(e.message, "error"); }
  };
  ...
  const handleAdminGive = async () => {
    const amt = parseFloat(adminGive.amount);
    if (!adminGive.username || !amt) return notify("Заполните поля", "error");
    try {
      const [target] = await sb(`users?username=eq.${adminGive.username}&select=*`);
      if (!target) return notify("Пользователь не найден", "error");
      await sb(`users?username=eq.${adminGive.username}`, {
        method: "PATCH",
        body: JSON.stringify({ balance: target.balance + amt })
      });
      await sb("transactions", {
        method: "POST", prefer: "return=minimal",
        body: JSON.stringify({ id: genId(), username: adminGive.username, type: "bonus", amount: amt, description: `👑 Начислено администратором`, date: nowStr() })
      });
      notify(`Начислено ${fmt(amt)} → ${target.fullname}`);
      setAdminGive({ username: "", amount: "" });
      loadAllUsers();
    } catch (e) { notify(e.message, "error"); }
  };
  ...
  const handleAdminDeduct = async () => {
    ...
    try {
      ...
      await sb(`users?username=eq.${adminGive.username}`, {
        method: "PATCH",
        body: JSON.stringify({ balance: Math.max(0, target.balance - amt) })
      });
      await sb("transactions", {
        method: "POST", prefer: "return=minimal",
        body: JSON.stringify({ id: genId(), username: adminGive.username, type: "out", amount: -amt, description: `👑 Списано администратором`, date: nowStr() })
      });
      notify(`Списано ${fmt(amt)} у ${target.fullname}`);
      setAdminGive({ username: "", amount: "" });
      loadAllUsers();
    } catch (e) { notify(e.message, "error"); }
  };
  ...
  if (!session || !user) {
    return (
      <div style={S.root}>
        {notification && <div style={{ ...S.notif, background: notification.type === "error" ? "#ff4d4d" : "#00d68f" }}>{notification.msg}</div>}
        <div style={S.authWrap}>
          <div style={S.authCard}>
            <div style={S.logo}><span style={S.logoIcon}>⛁</span><span style={S.logoText}>QAZAQ<b>BANK</b></span></div>
            <p style={S.tagline}>Виртуальный банк • Реальный опыт</p>
            ...
            {authPage === "register" && <>
              <input style={S.inp} placeholder="Полное имя" value={authForm.fullname} onChange={e => setAuthForm({ ...authForm, fullname: e.target.value })} />
              <input style={S.inp} placeholder="Дата рождения" type="date" value={authForm.birthdate} onChange={e => setAuthForm({ ...authForm, birthdate: e.target.value })} />
            </>}
            <input style={S.inp} placeholder="Логин" value={authForm.username} onChange={e => setAuthForm({ ...authForm, username: e.target.value })} />
            <input style={S.inp} type="password" placeholder="Пароль" value={authForm.password} onChange={e => setAuthForm({ ...authForm, password: e.target.value })}
              onKeyDown={e => e.key === "Enter" && (authPage === "login" ? handleLogin() : handleRegister())} />
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.appWrap}>
        {/* Sidebar */}
        <nav style={S.sidebar}>
          <div style={S.sidebarLogo}>⛁ QAZAQBANK</div>
          <div style={S.userInfo}>
            <div style={{ ...S.avatar, background: isAdmin ? "linear-gradient(135deg,#f9ca24,#f0932b)" : "linear-gradient(135deg,#00b894,#00d68f)" }}>
              {user.fullname[0]}
            </div>
            <div>
              <div style={S.userName}>{user.fullname} {isAdmin && <span style={S.adminBadge}>ADMIN</span>}</div>
              <div style={S.userLogin}>@{user.username}</div>
            </div>
          </div>
          {navItems.map(([p, icon, label]) => (
            <button key={p} style={{ ...S.navBtn, ...(page === p ? S.navBtnActive : {}) }} onClick={() => setPage(p)}>
              <span>{icon}</span> {label}
            </button>
          ))}
          <button style={S.navBtnLogout} onClick={handleLogout}>⎋ Выйти</button>
        </nav>
        ...
        <main style={S.content}>
          ...
          {page === "home" && (
            <div>
              <h1 style={S.pageTitle}>Привет, {user.fullname.split(" ")[0]}! 👋</h1>
              <div style={S.balanceCard}>
                <div style={S.balanceLabel}>ОСНОВНОЙ СЧЁТ</div>
                <div style={S.balanceAmount}>{fmt(user.balance)}</div>
              ...
              <div style={S.statsRow}>
                {[["💳", credits.length, "Кредитов"], ["🏦", deposits.length, "Вкладов"], ["📋", txList.length, "Операций"]].map(([icon, val, label]) => (
              ...
              <div style={S.earnSection}>
                <h2 style={S.secTitle}>💰 Клик-заработок</h2>
                <p style={S.earnDesc}>Каждый клик = 1 ₸ • Лимит 100 кликов, затем 30 сек перерыв</p>
                <div style={S.earnRow}>
                  <button style={{ ...S.earnBtn, ...(clickAnim ? S.earnBtnAnim : {}), ...(earnCooldown ? S.earnBtnOff : {}) }}
                    onClick={handleEarn} disabled={earnCooldown}>
                    {earnCooldown ? "⏳ Перерыв..." : "💰 Кликай!"}
                  </button>
                  <div style={S.earnCounter}>
                    <div style={S.earnVal}>{clickCount}/100</div>
                    <div style={S.statLabel}>кликов</div>
                  </div>
                </div>
              </div>
              <h2 style={S.secTitle}>Последние операции</h2>
              <div style={S.txList}>
                {txList.slice(0, 5).map(tx => <TxRow key={tx.id} tx={tx} />)}
                {txList.length === 0 && <p style={S.empty}>Операций нет</p>}
              </div>
          ...
          {page === "transfer" && (
            <div>
              <h1 style={S.pageTitle}>Перевод денег</h1>
              <div style={S.formCard}>
                ...
                <input style={S.inp2} placeholder="За ужин..." value={tfForm.note} onChange={e => setTfForm({ ...tfForm, note: e.target.value })} />
                {tfError && <p style={S.err}>{tfError}</p>}
                <p style={S.hint}>Доступно: <b>{fmt(user.balance)}</b></p>
                <button style={S.btnPrimary} onClick={handleTransfer} disabled={loading}>Отправить</button>
              </div>
            </div>
          )}
          ...
          {page === "statement" && (
            <div>
              <h1 style={S.pageTitle}>Выписка по счёту</h1>
              <div style={S.stmtHeader}>
                <span>Операций: {txList.length}</span>
                <span>Баланс: {fmt(user.balance)}</span>
              </div>
              <div style={S.txList}>
                {txList.map(tx => <TxRow key={tx.id} tx={tx} full />)}
                {txList.length === 0 && <p style={S.empty}>Операций нет</p>}
          ...
          {page === "credit" && (
            <div>
              <h1 style={S.pageTitle}>Кредиты</h1>
              {credits.length > 0 && (
                <div style={S.grid}>
                  {credits.map(c => (
                    <div key={c.id} style={S.creditCard}>
                      <div style={S.cardTop}><span style={S.cid}>#{c.id}</span><span style={S.badge}>Активный</span></div>
                      <div style={S.bigAmt}>{fmt(c.amount)}</div>
                      <div style={S.cinfo}><span>Остаток: <b style={{ color: "#ff6b6b" }}>{fmt(c.remaining)}</b></span><span>Взнос: {fmt(c.monthly)}</span></div>
                      <div style={S.cinfo}><span>Срок: {c.months} мес.</span><span>Выплат: {c.paid}</span></div>
              ...
              <h2 style={S.secTitle}>Оформить кредит</h2>
              <div style={S.formCard}>
                <Label>Сумма</Label>
                <input style={S.inp2} type="number" placeholder="100 000" value={crForm.amount} onChange={e => setCrForm({ ...crForm, amount: e.target.value })} />
                <Label>Срок</Label>
                ...
                {crForm.amount > 0 && (
                  <div style={S.calcBox}>
                    Ставка: <b>18%</b> • Ежемесячно: <b>{fmt(parseFloat(crForm.amount) * (0.18 / 12) / (1 - Math.pow(1 + 0.18 / 12, -parseInt(crForm.months))))}</b>
                  </div>
                )}
                {crError && <p style={S.err}>{crError}</p>}
                <button style={S.btnPrimary} onClick={handleCredit} disabled={loading}>Оформить кредит</button>
          ...
          {page === "deposit" && (
            <div>
              <h1 style={S.pageTitle}>Вклады</h1>
              {deposits.length > 0 && (
                <div style={S.grid}>
                  {deposits.map(d => (
                    <div key={d.id} style={{ ...S.creditCard, borderColor: "#00d68f33" }}>
                      <div style={S.cardTop}><span style={S.cid}>#{d.id}</span><span style={{ ...S.badge, background: "#00d68f22", color: "#00d68f" }}>Активный</span></div>
                      <div style={S.bigAmt}>{fmt(d.amount)}</div>
                      <div style={S.cinfo}><span>Ставка: <b style={{ color: "#00d68f" }}>{d.rate}%</b></span><span>Срок: {d.months} мес.</span></div>
                      <div style={S.cinfo}><span>Доход: <b style={{ color: "#00d68f" }}>+{fmt(d.profit)}</b></span></div>
                      <button style={S.btnGreen} onClick={() => handleCloseDeposit(d)}>Закрыть +{fmt(d.profit)}</button>
                    </div>
                  ))}
                </div>
              )}
              <h2 style={S.secTitle}>Открыть вклад</h2>
              <div style={S.formCard}>
                <Label>Сумма</Label>
                <input style={S.inp2} type="number" placeholder="10 000" value={depForm.amount} onChange={e => setDepForm({ ...depForm, amount: e.target.value })} />
                <Label>Срок</Label>
          ...
          {page === "cards" && (
            <div>
              <h1 style={S.pageTitle}>Мои карты</h1>
              <div style={S.virtualCard}>
                <div style={S.vcChip}>▬▬</div>
                <div style={S.vcNum}>{user.card_number?.match(/.{1,4}/g)?.join("  ")}</div>
                <div style={S.vcRow}>
              ...
              <div style={S.formCard}>
                {[["Номер", user.card_number?.match(/.{1,4}/g)?.join(" ")], ["CVV", "•••"], ["Срок", "12/29"], ["Статус", "✅ Активна"], ["Тип", "VISA Virtual"]].map(([k, v]) => (
                  <div key={k} style={{ color: "#aaa", fontSize: 14, marginBottom: 8 }}><b style={{ color: "#c8d0dc" }}>{k}:</b> {v}</div>
                ))}
              </div>
            </div>
          )}
          ...
          {page === "admin" && isAdmin && (
            <div>
              <h1 style={S.pageTitle}>👑 Админ панель</h1>

              {/* Give/Deduct money */}
              <div style={S.formCard}>
                <h2 style={{ ...S.secTitle, marginTop: 0 }}>Управление балансом</h2>
                <Label>Логин пользователя</Label>
                <input style={S.inp2} placeholder="username" value={adminGive.username} onChange={e => setAdminGive({ ...adminGive, username: e.target.value })} />
                <Label>Сумма (₸)</Label>
                <input style={S.inp2} type="number" placeholder="0" value={adminGive.amount} onChange={e => setAdminGive({ ...adminGive, amount: e.target.value })} />
                <div style={{ display: "flex", gap: 10 }}>
                  <button style={{ ...S.btnPrimary, flex: 1 }} onClick={handleAdminGive}>➕ Начислить</button>
                  <button style={{ ...S.btnPrimary, flex: 1, background: "linear-gradient(135deg,#ff4d4d,#ff6b6b)" }} onClick={handleAdminDeduct}>➖ Списать</button>
                </div>
              </div>

              {/* Users list */}
              <h2 style={S.secTitle}>Все пользователи ({allUsers.length})</h2>
              <div style={S.txList}>
                {allUsers.map(u => (
                  <div key={u.username} style={{ ...S.txRow, flexWrap: "wrap", gap: 8 }}>
                    <div style={{ ...S.avatar, width: 36, height: 36, fontSize: 14, flexShrink: 0 }}>{u.fullname[0]}</div>
                    <div style={{ flex: 1 }}>
                      <div style={S.txDesc}>{u.fullname} {u.is_admin && <span style={S.adminBadge}>ADMIN</span>}</div>
                      <div style={S.txDate}>@{u.username} • {u.birthdate}</div>
                    </div>
                    <div style={{ color: "#00d68f", fontWeight: 700 }}>{fmt(u.balance)}</div>
                    <button style={{ ...S.btnRed, width: "auto", padding: "6px 12px", fontSize: 12 }} onClick={() => viewUserTx(u.username)}>
                      История
                    </button>
                  </div>
                ))}
              ...
              {selectedUser && (
                <div style={S.modal}>
                  <div style={S.modalCard}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                      <h3 style={{ color: "#e8eaf0", margin: 0 }}>История @{selectedUser}</h3>
                      <button style={{ background: "none", border: "none", color: "#ff6b6b", fontSize: 20, cursor: "pointer" }} onClick={() => setSelectedUser(null)}>✕</button>
                    </div>
...
function TxRow({ tx, full }) {
  ...
  return (
    <div style={S.txRow}>
      ...
      <div style={S.txMeta}>
        <div style={S.txDesc}>{tx.description}</div>
        <div style={S.txDate}>{full && `ID: ${tx.id} • `}{tx.date}</div>
      </div>
      <div style={{ ...S.txAmt, color: pos ? "#00d68f" : "#ff6b6b" }}>{pos ? "+" : ""}{new Intl.NumberFormat("kk-KZ", { minimumFractionDigits: 2 }).format(Math.abs(tx.amount))} ₸</div>
    </div>
  );
...
// ── Styles ────────────────────────────────────────────────────────────────────
const S = {
  root: { minHeight: "100vh", background: "#0a0d14", color: "#e8eaf0", fontFamily: "'Segoe UI', system-ui, sans-serif" },
  notif: { position: "fixed", top: 20, right: 20, zIndex: 9999, padding: "12px 24px", borderRadius: 12, color: "#fff", fontWeight: 600, fontSize: 14, boxShadow: "0 4px 20px #0008" },
  loadingBar: { position: "fixed", top: 0, left: 0, right: 0, height: 3, background: "linear-gradient(90deg,#00b894,#00d68f)", zIndex: 9999, animation: "pulse 1s infinite" },

  authWrap: { display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: 20 },
  authCard: { background: "#111827", border: "1px solid #1f2d44", borderRadius: 24, padding: "48px 40px", width: "100%", maxWidth: 400, boxShadow: "0 20px 80px #000a" },
  logo: { display: "flex", alignItems: "center", gap: 10, marginBottom: 8 },
  logoIcon: { fontSize: 32 },
  logoText: { fontSize: 22, letterSpacing: 2 },
  tagline: { color: "#5a6a80", fontSize: 13, marginBottom: 28 },
  inp: { width: "100%", padding: "13px 16px", background: "#0d1929", border: "1px solid #1f2d44", borderRadius: 10, color: "#e8eaf0", fontSize: 15, marginBottom: 10, boxSizing: "border-box", outline: "none" },
  err: { color: "#ff6b6b", fontSize: 13, margin: "4px 0 10px" },
  btnPrimary: { width: "100%", padding: 14, background: "linear-gradient(135deg,#00b894,#00d68f)", border: "none", borderRadius: 12, color: "#000", fontSize: 15, fontWeight: 700, cursor: "pointer", marginBottom: 10 },
  btnGhost: { width: "100%", padding: 12, background: "transparent", border: "1px solid #1f2d44", borderRadius: 12, color: "#5a6a80", fontSize: 14, cursor: "pointer" },

  appWrap: { display: "flex", minHeight: "100vh" },
  sidebar: { width: 240, background: "#0d1117", borderRight: "1px solid #1f2d44", padding: "24px 16px", display: "flex", flexDirection: "column", gap: 4, flexShrink: 0 },
  sidebarLogo: { fontSize: 13, fontWeight: 800, letterSpacing: 2, color: "#00d68f", marginBottom: 20, padding: "0 8px" },
  userInfo: { display: "flex", alignItems: "center", gap: 10, padding: "12px 8px 20px", borderBottom: "1px solid #1f2d44", marginBottom: 10 },
  avatar: { width: 40, height: 40, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 18, color: "#000", flexShrink: 0 },
  userName: { fontSize: 13, fontWeight: 600 },
  userLogin: { fontSize: 11, color: "#5a6a80" },
  adminBadge: { background: "linear-gradient(135deg,#f9ca24,#f0932b)", color: "#000", fontSize: 9, fontWeight: 800, padding: "2px 6px", borderRadius: 20, marginLeft: 4 },
  navBtn: { display: "flex", alignItems: "center", gap: 10, padding: "11px 12px", borderRadius: 10, border: "none", background: "transparent", color: "#8899aa", fontSize: 14, cursor: "pointer" },
  navBtnActive: { background: "#00d68f18", color: "#00d68f" },
  navBtnLogout: { marginTop: "auto", display: "flex", alignItems: "center", gap: 10, padding: "11px 12px", borderRadius: 10, border: "none", background: "transparent", color: "#ff6b6b", fontSize: 14, cursor: "pointer" },

  content: { flex: 1, padding: "40px 48px", overflowY: "auto" },
  pageTitle: { fontSize: 28, fontWeight: 800, marginBottom: 24 },
  secTitle: { fontSize: 17, fontWeight: 700, margin: "24px 0 12px", color: "#c8d0dc" },

  balanceCard: { background: "linear-gradient(135deg,#0a2a1e,#0d3326)", border: "1px solid #00d68f33", borderRadius: 20, padding: 28, marginBottom: 20 },
  balanceLabel: { fontSize: 11, color: "#00d68f88", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 },
  balanceAmount: { fontSize: 40, fontWeight: 900, color: "#00d68f", marginBottom: 14 },
  balanceRow: { display: "flex", justifyContent: "space-between", color: "#5a8a70", fontSize: 14 },

  statsRow: { display: "flex", gap: 14, marginBottom: 24 },
  statBox: { flex: 1, background: "#111827", border: "1px solid #1f2d44", borderRadius: 16, padding: 18, textAlign: "center" },
  statIcon: { fontSize: 22, marginBottom: 6 },
  statVal: { fontSize: 28, fontWeight: 900 },
  statLabel: { fontSize: 11, color: "#5a6a80", marginTop: 4 },

  earnSection: { background: "#111827", border: "1px solid #1f2d44", borderRadius: 20, padding: 22, marginBottom: 24 },
  earnDesc: { color: "#5a6a80", fontSize: 13, marginBottom: 14 },
  earnRow: { display: "flex", alignItems: "center", gap: 20 },
  earnBtn: { padding: "14px 28px", background: "linear-gradient(135deg,#f9ca24,#f0932b)", border: "none", borderRadius: 14, fontSize: 17, fontWeight: 800, cursor: "pointer", color: "#000", transition: "transform .1s" },
  earnBtnAnim: { transform: "scale(0.93)" },
  earnBtnOff: { background: "#1f2d44", color: "#5a6a80", cursor: "not-allowed" },
  earnCounter: { textAlign: "center" },
  earnVal: { fontSize: 22, fontWeight: 900, color: "#f9ca24" },

  txList: { background: "#111827", border: "1px solid #1f2d44", borderRadius: 16, overflow: "hidden" },
  txRow: { display: "flex", alignItems: "center", gap: 12, padding: "13px 18px", borderBottom: "1px solid #1a2130" },
  txBadge: { width: 34, height: 34, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, fontWeight: 900, flexShrink: 0 },
  txMeta: { flex: 1, minWidth: 0 },
  txDesc: { fontSize: 14, color: "#c8d0dc", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
  txDate: { fontSize: 11, color: "#5a6a80", marginTop: 2 },
  txAmt: { fontSize: 14, fontWeight: 700, flexShrink: 0 },
  empty: { color: "#5a6a80", padding: 20, textAlign: "center" },

  formCard: { background: "#111827", border: "1px solid #1f2d44", borderRadius: 20, padding: 26, maxWidth: 500 },
  inp2: { width: "100%", padding: "12px 14px", background: "#0d1929", border: "1px solid #1f2d44", borderRadius: 10, color: "#e8eaf0", fontSize: 15, boxSizing: "border-box", outline: "none", marginBottom: 4 },
  sel: { width: "100%", padding: "12px 14px", background: "#0d1929", border: "1px solid #1f2d44", borderRadius: 10, color: "#e8eaf0", fontSize: 15, outline: "none" },
  hint: { fontSize: 13, color: "#5a6a80", margin: "10px 0" },
  calcBox: { background: "#0d1929", border: "1px solid #1f2d44", borderRadius: 10, padding: "11px 14px", margin: "10px 0", fontSize: 14, color: "#8899aa" },
  stmtHeader: { display: "flex", justifyContent: "space-between", color: "#5a6a80", fontSize: 13, marginBottom: 12 },

  grid: { display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(270px,1fr))", gap: 14, marginBottom: 24 },
  creditCard: { background: "#111827", border: "1px solid #ff6b6b33", borderRadius: 16, padding: 18 },
  cardTop: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 },
  cid: { fontSize: 11, color: "#5a6a80", fontFamily: "monospace" },
  badge: { fontSize: 11, background: "#ff6b6b22", color: "#ff6b6b", padding: "3px 8px", borderRadius: 20 },
  bigAmt: { fontSize: 22, fontWeight: 900, margin: "8px 0" },
  cinfo: { display: "flex", justifyContent: "space-between", fontSize: 12, color: "#8899aa", marginBottom: 5 },
  btnRed: { width: "100%", padding: "9px", background: "#ff6b6b22", border: "1px solid #ff6b6b44", borderRadius: 8, color: "#ff6b6b", fontSize: 13, cursor: "pointer", marginTop: 10 },
  btnGreen: { width: "100%", padding: "9px", background: "#00d68f22", border: "1px solid #00d68f44", borderRadius: 8, color: "#00d68f", fontSize: 13, cursor: "pointer", marginTop: 10 },

  virtualCard: { background: "linear-gradient(135deg,#1a1a2e,#16213e,#0f3460)", borderRadius: 20, padding: "26px 30px", maxWidth: 370, marginBottom: 22, border: "1px solid #ffffff11", boxShadow: "0 20px 60px #000a" },
  vcChip: { fontSize: 22, color: "#f9ca24", marginBottom: 18 },
  vcNum: { fontSize: 19, fontFamily: "monospace", letterSpacing: 3, color: "#fff", marginBottom: 22 },
  vcRow: { display: "flex", alignItems: "flex-end", gap: 20 },
  vcLbl: { fontSize: 10, color: "#ffffff66", textTransform: "uppercase", letterSpacing: 1 },
  vcName: { fontSize: 13, color: "#fff", fontWeight: 600 },
  vcVisa: { marginLeft: "auto", fontSize: 18, fontStyle: "italic", fontWeight: 900, color: "#fff" },

  modal: { position: "fixed", inset: 0, background: "#000a", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 },
  modalCard: { background: "#111827", border: "1px solid #1f2d44", borderRadius: 20, padding: 24, width: "100%", maxWidth: 500, maxHeight: "80vh", overflowY: "auto" },
};
